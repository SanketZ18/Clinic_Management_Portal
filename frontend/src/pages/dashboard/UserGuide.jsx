...
const sections = [
  ...
  },
  {
    ...
    steps: [
      ...
      'The previous prescription is not overwritten and old clinical notes or remedies are not automatically copied into the new visit. Saving the new prescription adds this consultation to the same patient record.',
      'If the search returns no match, continue by entering the patient details manually. The patient record will be created when the prescription is saved.',
    ...
    items: [
      ...
      ['Search Patient', 'Looks up an existing patient using both the full name and mobile number. Searching with only one of these values is not supported.'],
      ['Search result', 'Shows whether the patient was found, the number of previous visits, and—when available—the date, complaint, and first remedy from the last visit. This is a quick identity and context check, not a replacement for the full history.'],
      ['Auto-fill Form', 'Copies the matched patient details into the new visit form. Always verify the information before continuing, especially the name and mobile number.'],
      ['Continue with old patient', 'After auto-fill, enter only the new consultation information and save or share the prescription. The new visit is appended to the existing patient record.'],
      ['No match found', 'Enter the patient details manually and continue as a new patient. The record is created when the prescription is saved.'],
      ['Full previous history', 'Use Patient Log / Report Generation and select Patient History PDF or a rowâ€™s History button when you need the patientâ€™s complete chronological visit history.'],
    ],
  },
  {
